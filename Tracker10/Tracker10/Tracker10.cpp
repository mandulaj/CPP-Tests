...
int OP_1 = 2;
int OP_2 = 5;
const string trackbarWindowName = "Color";
...
void createTrackbars(){
	...
	//sprintf( TrackbarName, "OP_1", OP_1);
	//sprintf( TrackbarName, "OP_2", OP_2);
	...
	//createTrackbar( "OP_1", trackbarWindowName, &B_MAX, OP_1);
	//createTrackbar( "OP_2", trackbarWindowName, &B_MAX, OP_2);
...
int main() {
	cv::VideoCapture capWebcam("http://10.0.0.1:8080/?action=stream?dummy=param.mjpg");		// declare a VideoCapture object and associate to webcam, 0 => use 1st webcam "http://10.0.0.1:8080/?action=stream?dummy=param.mjpg"
	...
	while(charCheckForEscKey != 27) {		// for each frame (until the Esc key is pressed) . . .
		...
		cv::inRange(matOriginal,				// funcion input
					cv::Scalar(B_MIN,G_MIN,R_MIN),		// min filtering value (if greater than or equal to this) (in BGR format)
					cv::Scalar(B_MAX,G_MAX,R_MAX),	// max filtering value (and if less than this) (in BGR format)
					matProcessed);				// function output

		morphOps(matProcessed);
